...
export interface TrackedResource {
  ...
  kind?: string;
  /**
   * Entity tag is used for comparing two or more entities from the same requested resource. ETags
   * may be returned for individual resources, and then sent via If-Match / If-None-Match headers
...
/**
 * Entity tag is used for comparing two or more entities from the same requested resource. ETags
 * may be returned for individual resources, and then sent via If-Match / If-None-Match headers for
 * concurrency control.
 */
 ...
 */
export type AutomationActionUnion = AutomationAction | AutomationActionLogicApp | AutomationActionEventHub | AutomationActionSecurityEmail | AutomationActionWorkspace;
...
export interface Automation extends TrackedResource {
   ...
   * within at least one ruleset, are true.
   */
...
export interface AutomationActionEventHub {
  ...
  actionType: "EventHub";
  /**
   * The Event Hub Azure Resource ID.
   */
...
/**
 * An action that triggers sending of Security emails. Emails can be sent to users with specific
 * roles on the subscription (for example, subscription owners) or to user-provided emails.
 */
export interface AutomationActionSecurityEmail {
  /**
   * Polymorphic Discriminator
   */
  actionType: "SecurityEmail";
  /**
   * A list of user roles which indicate the relevant users that will receive the event data via
   * email.
   */
  subscriptionRbacRoles?: SubscriptionRbacRoles[];
  /**
   * A list of email addresses that should receive the event data.
   */
...
export interface AutomationActionWorkspace {
  ...
  /**
   * The Log Analytics Workspace ID in a GUID format.
   */
  workspaceId?: string;
}
 ...
 * Possible values include: 'AccountAdmin', 'ServiceAdmin', 'Owner', 'Contributor'
 * @readonly
 * @enum {string}
 */
export type SubscriptionRbacRoles = 'AccountAdmin' | 'ServiceAdmin' | 'Owner' | 'Contributor';
