...
export interface SpellCheckClientSpellCheckerOptionalParams extends msRest.RequestOptionsBase {
   ...
   * session or a page load. The following are the possible values. 1) Edit—The request is from an
   * interactive session 2) Load—The request is from a page load. Possible values include: 'Edit',
   * 'Load'
   */
   ...
   * values are case insensitive). The default is Proof. 1) Proof—Finds most spelling and grammar
   * mistakes. 2) Spell—Finds most spelling mistakes but does not find some of the grammar errors
