...
/**
 * An interface representing ImputeStrategy.
 */
export interface ImputeStrategy {
  /**
   * The method that is used to impute missing values in the given time series. Possible values
   * include: 'auto', 'previous', 'linear', 'fixed', 'zero', 'notFill'
   */
  imputeMode: ImputeMode;
  /**
   * Optional argument, if imputeMode is set to 'fixed', then this value is used to impute missing
   * values.
   */
  imputeValue?: number;
}
...
export interface DetectRequest {
  ...
  sensitivity?: number;
  /**
   * Optional argument, indicates how to impute the missing values in the series.
   */
...
export interface EntireDetectResponse {
   ...
   * **NOTE: This property will not be serialized. It can only be populated by the server.**
   */
  readonly isNegativeAnomaly?: boolean[];
  /**
   ...
   * **NOTE: This property will not be serialized. It can only be populated by the server.**
   */
  readonly isPositiveAnomaly?: boolean[];
  /**
   * Severity contains the severity score for each input point. For a normal point, the severity
   * score is 0. For an anomaly point, the severity score is in (0, 1] that indicates the severity
   * of the anomaly.
   * **NOTE: This property will not be serialized. It can only be populated by the server.**
   */
...
export interface LastDetectResponse {
   ...
   * **NOTE: This property will not be serialized. It can only be populated by the server.**
   */
  readonly suggestedWindow?: number;
  /**
  ...
  readonly isPositiveAnomaly?: boolean;
  /**
   * Severity score of the latest point. For a normal point, the severity score is 0. For an
   * anomaly point, the severity score is in (0, 1] that indicates the severity of the anomaly.
   * **NOTE: This property will not be serialized. It can only be populated by the server.**
   */
  readonly severity?: number;
}
...
export interface ChangePointDetectRequest {
   ...
   * 'monthly', 'weekly', 'daily', 'hourly', 'perMinute', 'perSecond', 'microsecond'
   */
  granularity: TimeGranularity;
  /**
   * Custom Interval is used to set non-standard time interval, for example, if the series is 5
 ...
 * @enum {string}
 */
export type TimeGranularity = 'yearly' | 'monthly' | 'weekly' | 'daily' | 'hourly' | 'minutely' | 'secondly' | 'microsecond';

/**
 * Defines values for ImputeMode.
 * Possible values include: 'auto', 'previous', 'linear', 'fixed', 'zero', 'notFill'
 * @readonly
 * @enum {string}
 */
